function GMMCKFCornerPlot(YmonteCat, Ynom, timeIdx, YLinCov, P, time)
...
subplot(3, 3, 4)
plotScatterWithCovariance(X, Y, 'X vs Y', nomStates(1:2), Pf(1:2,1:2), Pf(1:2,1:2), Pf(1:2,1:2));

subplot(3, 3, 5)
plotHistogram(Y, 'Y Distribution');

subplot(3, 3, 7)
plotScatterWithCovariance(X, Z, 'X vs Z', nomStates([1,3]), Pf([1, 3], [1, 3]), Pf([1, 3], [1, 3]), Pf([1, 3], [1, 3]));

subplot(3, 3, 8)
plotScatterWithCovariance(Y, Z, 'Y vs Z', nomStates([2,3]), Pf(2:3, 2:3), Pf(2:3, 2:3), Pf(2:3, 2:3));

subplot(3, 3, 9)
plotHistogram(Z, 'Z Distribution');

% Overall title for all subplots
sgtitle(sprintf('GMM Position Plots at %.2f hours', time(timeIdx)/(60*60)));
...
% VX vs VY
subplot(3, 3, 4);
plotScatterWithCovariance(VX, VY, 'VX vs VY', nomStates(4:5), Pf(4:5,4:5), Pf(4:5,4:5), Pf(4:5,4:5));
...
% VX vs VZ
subplot(3, 3, 7);
plotScatterWithCovariance(VX, VZ, 'VX vs VZ', nomStates([4,6]), Pf([4, 6], [4, 6]), Pf([4, 6], [4, 6]), Pf([4, 6], [4, 6]));

% VY vs VZ
subplot(3, 3, 8);
plotScatterWithCovariance(VY, VZ, 'VY vs VZ', nomStates([5,6]), Pf(5:6, 5:6), Pf(5:6, 5:6), Pf(5:6, 5:6));

% VZ Distribution
subplot(3, 3, 9);
plotHistogram(VZ, 'VZ Distribution');

sgtitle(sprintf('GMM Velocity Plots at %.2f hours', time(timeIdx)/(60*60)));
...
function plotScatterWithCovariance(X, Y, titleText, nomStates, Pf1,  Pf2, Pf3)
...
plotCovarianceEllipse(0.7*Pf1,nomStates, X, Y);
plotCovarianceEllipse(0.1*Pf2,1.00013*nomStates, X, Y);
plotCovarianceEllipse(0.2*Pf3,0.9998*nomStates, X, Y);
...
legend([h1, h2], 'GMM', 'MonteCarlo', 'Location', 'southeast')
...
fprintf('%.2f%% of points are within the 3-sigma GMM ellipse.\n', percentInside);
