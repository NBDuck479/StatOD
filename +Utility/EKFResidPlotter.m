function [fig] = EKFResidPlotter(preFit_res, resid_pfHist, fig, simuTime)
  ...
  %  [overlapTimeIdn, ~] = ismember(simuTime, obsHist.time);
    ...
    xlabel('Time [hours]')
    ylim([-1*10^-5 1*10^-5])
    sgtitle('Post-Fit EKF Residuals')
    grid on
    ...
    figure(fig)
    subplot(2,1,1)
    histogram(fillpreFit_res_rho, 'BinWidth', .0005)
    xlim([-0.01, 0.01])
    grid on
    ylabel('Count')
    title('Range')
    ...
    grid on
    xlim([-0.1*10^-4, 0.1*10^-4])
    ylabel('Count')
    xlabel('Residual')
    title('Range Rate')
    
    sgtitle('EKF Pre-Fit Residuals Histogram')
    ...
    subplot(2,1,2)
    histogram(fillpostFit_res_rhoDot, 'BinWidth', 0.0000002)
    grid on
    xlim([-0.000006 0.000006])
    ylabel('Count')
    xlabel('Residual')
    title('Range Rate')
    
    sgtitle('EKF Post-Fit Residuals Histogram')
